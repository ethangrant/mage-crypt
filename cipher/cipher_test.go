...
func TestChacha20poly1305Decrypt(t *testing.T) {
	...
	for _, testcase := range testCases {
		plaintext, err := chacha20poly1305Decrypt(testcase.ciphertext, testcase.key)
		if err != nil {
	...
	for _, testcase := range testCases {
		ciphertext, err := chacha20poly1305Encrypt(testcase.plaintext, testcase.key, "1")
		if err != nil {
			fmt.Println(err.Error())
			return
		}

		plaintext, err := chacha20poly1305Decrypt(ciphertext, testcase.key)
		if err != nil {
			fmt.Println(err.Error())
			return
		}
...
{ciphertext: "1:3:W8B7ZUXGY4cRs7ZxU9HQ9vFCCDC7TFlp:eW1MUF6WPB7I6IpdNPJYUS9goWG0pehi9+ePIM5Pb8kdwYAElSRaod6FyOSNjoiqDCUTrYfoOyXAJkSqPgxwyEB90Kr8AeuESo026kvHXhyyYBBXhQZ5owYpGfYLPLj0", plaintext: "You've got to ask yourself one question: 'Do I feel lucky?' Well, do you, punk?", key: "e331b68cf8e8646f09e6ddcad2d32d83"},
	...
	for _, testcase := range testCases {
		plaintext, err := rijandel256Decrypt(testcase.ciphertext, testcase.key)
		if err != nil {
			fmt.Println(err.Error())
			return
		}
	...
	for _, testcase := range testCases {
		ciphertext, err := rijandel256Encrypt(testcase.plaintext, testcase.key, "1")
		if err != nil {
			fmt.Println(err.Error())
			return
		}

		plaintext, err := rijandel256Decrypt(ciphertext, testcase.key)
		if err != nil {
			fmt.Println(err.Error())
			return
		}
